...
#DAEMON=/bin/bash
DAEMON_USER=root
DAEMON_ARGS=/home/pi/dev/lightstrip/LDP8806/lightshow.py
...
do_start()
{
	# Logging via bash redirection taken from 
	...
	start-stop-daemon --start --background --make-pidfile --pidfile $PIDFILE --startas /bin/bash --test > /dev/null \
		|| return 1
